...
namespace StringInterpolation {
  class Program {
    static void Main(string[] args) {
      ...
      // One fix that's supposed to come in later is an improvement
      // to the syntactic nonsense with the backslash. The above 
      // can then be rewritten like this:
      //string betterInfo_ =
      //  $"Your order '{order.Name}' has {order.Items} item{order.Items == 1 ? "" : "s"}.";

      // It should also be possible to include specific formatting
      // locales in the future. Let's wait and see.
    }
  }
}
